...
@section('content')
    ...
    <div class="panel" x-data="{ itemType: '{{ old('type', 'note') }}' }">
        <h5 class="text-lg font-semibold dark:text-white-light mb-5">{{ __('ITEMS_ADD') }}</h5>
                        ...
                        <div>
                            ...
                            <select id="priority" name="priority" class="form-input">
                                @foreach (\App\Enums\TaskPriority::cases() as $case)
                                    <option value="{{ $case->value }}" @selected(old('priority', 'normal') === $case->value)>
                        ...
                        <div>
                            ...
                            <select id="task_status" name="task_status" class="form-input">
                                @foreach (\App\Enums\TaskStatus::cases() as $case)
                                    <option value="{{ $case->value }}" @selected(old('task_status', 'todo') === $case->value)>
                                        {{ ucfirst($case->value) }}</option>
                                @endforeach
                            </select>
                        </div>
