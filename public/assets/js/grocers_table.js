  ...
  Alpine.data('sorting', () => ({
    ...
    async init() {
      this.datatable = new simpleDatatables.DataTable('#myTable', {
        data: {
          headings: [
            `${Alpine.store('app').locale === 'en' ? 'ID' : 'رقم البطاقة'}`,
            `${
              Alpine.store('app').locale === 'en' ? 'Grocer Name' : 'اسم البقال'
            }`,
            `${
          ...
          data: await getGrocers() // Use the data fetching function
        },
        searchable: true,
        perPage: 10,
          ...
          // Add an extra column for 'Action' with a custom renderer
          {
            ...
            render: (data, td, rowIndex, cellIndex) => `
              <div class="flex gap-2">
                <a href="grocer?id=${data}" >
                <button type="submit" class="badge bg-warning">
                  <i class="ri-eye-line"></i>
                </button>
                </a>
                <form action="api/users/grocer/${data}" method="POST" class="badge bg-danger">
