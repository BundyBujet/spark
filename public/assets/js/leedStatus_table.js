document.addEventListener("alpine:init", () => {
  Alpine.data("sorting", () => ({
    ...
    async init() {
      const dataFetch = await getAllLeedStatus();
      ...
      this.datatable = new simpleDatatables.DataTable("#myTable", {
        ...
        data: {
          headings: [
            ...
            `${
              Alpine.store("app").locale === "en" ? "Status Name" : "اسم الحالة"
            }`,
            `${
          ...
          },
          {
            ...
            render: (data, td, rowIndex, cellIndex) => `
                <div class="flex gap-2">
                  <a href="edit-leedStatus?id=${data}" >
      ...
      this.datatable.on("datatable.page", async (page) => {
        ...
        if (page <= this.totalPages && !isPageFetched) {
          ...
          try {
            const { data } = await getAllLeedStatus(page);
            ...
            data.forEach((row) => {
              ...
              const ActionRow = `
            <div class="flex gap-2">
            <a href="edit-leedStatus?id=${userId}" >
            <button type="submit" class="badge bg-warning">
            <i class="text-xl ri-pencil-fill"></i>
            </button>
            </a>
...
async function showAlert(data) {
  new window.Swal({
    ...
    if (result.value) {
      ...
      try {
        const response = await fetch(`api/leedStatus/delete`, {
          method: "POST",
          credentials: "include",
        ...
        if (response.ok) {
          window.location.href = "/leedStatus";
        } else {
          window.location.href = "/leedStatus";
        }
      } catch (error) {
