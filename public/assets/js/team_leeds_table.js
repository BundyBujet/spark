document.addEventListener("alpine:init", () => {
  Alpine.data("filterLeedsTable", () => ({
    ...
    async init() {
      ...
      try {
        const filterDataResult = await this.fetchFirstPage();
    ...
    async handleSubmit(event) {
      ...
      if (
        this.validateDateRange(this.form.dateRange) &&
    ...
    async fetchFirstPage(page = 1, perPage = 10) {
      const [startDate, endDate] = this.form.dateRange.split(" to ");
      const response = await fetch(
        `/api/teams/leeds?page=${page}&perPage=${perPage}`,
        {
          method: "GET",
          headers: {
